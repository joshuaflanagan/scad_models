...
teensy_board_length = 35.56;
teensy_board_width = 17.84;
teensy_board_thickness = 1.57;
teensy_pin_separation = 2.54;
teensy_usb_height_off_board = 2.5;
// uncooment to debug module
//teensy_32(pin_holes=true, microcontroller=true, bounding_box=false);

module teensy_32(pin_holes=true, microcontroller=true, bounding_box=false){
...
usb_height = teensy_usb_height_off_board;

pin_separation=teensy_pin_separation;
pin_offset=pin_separation / 2;

top_pin_line = teensy_board_width - pin_offset;
bottom_pin_line = top_pin_line - (6 * pin_separation);
center_line = top_pin_line - (pin_separation * 3);

center_of_first_pin_to_center_of_reset = 29.97;

if (bounding_box){
  cube([teensy_board_length, teensy_board_width, teensy_board_thickness + usb_height]);
} else {
  color("green")
  ...
  if (microcontroller) {
    ...
    translate([teensy_board_length - 12.5, center_line, teensy_board_thickness])
    microcontroller();
  }

  translate([pin_offset + center_of_first_pin_to_center_of_reset,
             center_line,
             teensy_board_thickness])
  reset_button();
}
...
module usb_connector(bounding_box=false){
  ...
  translate([-usb_overhang, center_line - (usb_width / 2), teensy_board_thickness]){
    if (bounding_box){
        translate([
          0,
...
module pcb(pin_holes){
  linear_extrude(height=teensy_board_thickness)
  difference(){
    // Everything is specified relative to the top line (y=teensy_board_width).
    // As manufacturing_variance increases, everything moves up, adding
    // more empty board to the bottom.
    square([teensy_board_length, teensy_board_width]);
    if (pin_holes) {
